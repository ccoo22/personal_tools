...
"Usage: MetaboAnalyst_pca -i <file> --pcaplot <file> --biplot <file> --pca_coordinate <file> --sample_group_file <file> [--color <string> --show_confidence]
Options:
...
data <- read.table(inputfile, sep='\t', header = T, check.names=FALSE) # 读取定量数据，列为样本
sample.groups <- read.table(groupfile, sep = "\t", header = F, quote="", stringsAsFactors = FALSE) # 读取样本以及分组数据
# PCA分析要求
# （1）输入数据，行为特征值，列为样本
# （2）每一列不能存在缺失数据
