...
# 读入数据,data.frame格式
data          <- read.table(inputfile, header = T, sep = "\t", comment.char = "", check.names = F)
sample.groups <- read.table(groupfile, header = F, sep = "\t", quote = "")
...
{
    cat('[Error] exist unique sample, please check groupfile !\n')
    q()
}
if(group_count < 3)
{
...
# 结果列表，空矩阵
result <- matrix(, nrow = nrow(data), ncol = length(keep_column) + 2 + combine_count * 4) # 保留表头、P值/FDR、分组两两之间统计量
...
# （2）anova检测
col_names <- c(col_names, 'P-value(anova)', 'FDR P-value(anova)')
...
for(row in 1:nrow(data))
{
    ...
    data_normal      <- run_normal_test(y)                     # 正态分布检验
    result_tmp       <- c(annotation, pvalue, NA, data_normal) # 前面半部分结果
   
    # 填充前半部分结果
    result[row, 1:length(result_tmp) ] = result_tmp

}
# FDR矫正
result[, 'FDR P-value(anova)']       <- p.adjust(result[, 'P-value(anova)'], method = 'fdr')
write.table(result, output , sep = "\t", quote = F, row.names = F, col.names = TRUE )

# 差异ID
diff_id = result[,1][result[, 'P-value(anova)'] < 0.05]
