...
group_name = sapply(pca_data$Sample, function(sample_name){ 
        group = c(sample_name, rep('NO_GROUP', (ncol(group_data) - 1)  ))
        if(sample_name %in% group_data$Sample) group = group_data[which(group_data$Sample == sample_name), , drop=FALSE]
        group  
    } ) 
pca_data$Group = factor(unlist(group_name['Group', ]))
if(ncol(group_data) == 3) pca_data$Group2 = factor(unlist(group_name['Group2', ]))
