...
library(docopt)
"Usage: edger.r  -i <file> -o <dir> --case_group_name <string> --control_group_name <string> --case_sample_list <string> --control_sample_list <string> [--cor_file <file> --anno_col <string> --pvalue_cutoff <numeric> --log2fc_cutoff <numeric> --Rlib <dir>]
Options:
...
# (1) 差异算法1 The exact test is based on the qCML methods. 人们将其称为classic edgeR
# et <- exactTest(y)
# tTag = topTags(et, n=nrow(et))  # 取出所有差异结果 logFC    logCPM       PValue          FDR
# res <- as.data.frame(tTag)  # 差异分析结果 
# colnames(res) = c('log2FoldChange', 'baseMean', 'pvalue', 'padj')  # 修改一下列名，与deseq2保持一致
# res$baseMean = 2^res$baseMean  # 恢复原始平均表达量
# res = res[, c('baseMean', 'log2FoldChange', 'F', 'pvalue', 'padj')] # 调整一下顺序， 与deseq2一致

# (2) 差异算法2 Generalized linear models。 现在更推荐用这个
fit <- glmQLFit(y, design, robust=TRUE)
qlf <- glmQLFTest(fit)
tTag = topTags(qlf, n=nrow(qlf)) # logFC   logCPM          F       PValue        FDR。 其中logCPM是矫正后的表达量取均值，然后取log2
res <- as.data.frame(tTag)  # 差异分析结果 
colnames(res) = c('log2FoldChange', 'baseMean', 'F', 'pvalue', 'padj')  # 修改一下列名，与deseq2保持一致
