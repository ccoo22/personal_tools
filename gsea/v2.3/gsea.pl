...
# 定义 -> 核心变量
my $gsea                = "/home/genesky/software/gsea/4.3.2/gsea-cli.sh";
...
die help() if(defined $if_help or (not defined $input_matrix or not defined $sample_group or not defined $output_dir or (not defined $database and not defined $gmt_file)));
...
my @groups       = sort {$hashGroup{$a}{'Order'}<=>$hashGroup{$b}{'Order'}} keys %hashGroup; # 分组
my $group_count  = scalar(@groups); # 分组数量
my @samples      = map{ my @samples_tmp = sort keys %{$hashGroup{$_}{'Sample'}}; @samples_tmp; } @groups; # 样本
my @sample_groups = map{ my $group = $_; my @tmps = map{ $group } (sort keys %{$hashGroup{$group}{'Sample'}}); @tmps } @groups; # 每个样本所属分组
...
sub database_list{
    ...
    $hashDB{'Human_KEGG'} = "/home/genesky/database/gsea/msigdb_v2023.1.Hs_GMTs/c2.cp.kegg.v2023.1.Hs.symbols.gmt";
    $hashDB{'Human_GO'} = "/home/genesky/database/gsea/msigdb_v2023.1.Hs_GMTs/c5.go.v2023.1.Hs.symbols.gmt";
    $hashDB{'Human_HALLMARK'} = "/home/genesky/database/gsea/msigdb_v2023.1.Hs_GMTs/h.all.v2023.1.Hs.symbols.gmt";

    $hashDB{'Mouse_GO'}        = "/home/genesky/database/gsea/msigdb_v2023.1.Mm_GMTs/m5.go.v2023.1.Mm.symbols.gmt";
    $hashDB{'Mouse_Pathway'}   = "/home/genesky/database/gsea/msigdb_v2023.1.Mm_GMTs/m2.cp.v2023.1.Mm.symbols.gmt";
    $hashDB{'Mouse_HALLMARK'}   = "/home/genesky/database/gsea/msigdb_v2023.1.Mm_GMTs/mh.all.v2023.1.Mm.symbols.gmt";
...
sub choose_metric_para{
    ...
    {
        my @samples = keys %{$hashGroup->{$group}{'Sample'}};
...
sub read_sample_group{
    ...
    while(<GROUP>)
    {
        ...
        $hashGroup{$group}{'Sample'}{$sample}++;
        $hashGroup{$group}{'Order'} = $. if(not exists $hashGroup{$group}{'Order'});
    }
    close GROUP;
    return %hashGroup;    
}
...
Options:
    ...
    [必填]
         ...
         --sample_group/-s    样本分组文件，两列，第一列样本名，第二列分组名, 有表头。注：分组数量必须是2个。（流程会根据样本名，从input_matrix中提取需要的样本）
                              注意：分组名称不要包含空格
         --database/-d        数据库名称，例如：Human_KEGG
         --output_dir/-o      输出目录, 流程自己创建，不需要提前创建

    [选填]
         --gmt_file           自定义GSEA富集分析数据库文件路径， gmt格式。参考：人 hallmark /home/genesky/database/gsea/msigdb_v2023.1.Hs_GMTs/h.all.v2023.1.Hs.symbols.gmt
                              当输入gmt_file时，会忽略database物种。
