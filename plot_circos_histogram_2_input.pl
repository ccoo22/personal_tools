...
# 检测 -> 脚本输入
my ($input1, $input2, $output_dir, $prefix, $karyotype, $color1, $color2, $max, $if_help);
GetOptions(
    "input1=s"         => \$input1,
    "input2=s"         => \$input2,
...
die help() if(defined $if_help or (not defined $input1 or not defined $input2 or not defined $output_dir or not defined $prefix or not defined $karyotype));
###################################################################### 主程序
$color1 = 'vdred' if(not defined $color1);
$color2 = 'vdblue' if(not defined $color2);
...
print CONFIG "\t<plot>   \n";
print CONFIG "\t\ttype        = histogram    \n";
print CONFIG "\t\tfile        = $input2   \n";
print CONFIG "\t\torientation = in   \n";# 方向向外
print CONFIG "\t\textend_bin  = no   \n"; # 区域不自动连接
print CONFIG "\t\tthickness   = 2p   \n"; # 边框宽度
print CONFIG "\t\tr0          = 0.34r   \n"; # 直方图绘制范围
print CONFIG "\t\tr1          = 0.59r   \n";
print CONFIG "\t\tfill_color  = $color2    \n";# 填充色
print CONFIG "\t\tcolor       = $color2    \n";# 边框色
print CONFIG "\t\tmin         = 0   \n";# 坐标最小值
print CONFIG "\t\tmax         = $max   \n" if($max ne 'max');# 最表最大值
print CONFIG "\t\t<axes>   \n"; 
print CONFIG "\t\t\tshow      = data  \n"; 
...
Options:
         必填：
         --input1           要绘制的直方图文件,要符合circos直方图文件格式
         --input2           要绘制的直方图文件,要符合circos直方图文件格式
         --output_dir/-o    输出结果目录
         --prefix/-p        结果文件前缀
         --karyotype/-k     circos格式的染色体信息，例如：/home/ganb/soft/circos-0.69-5/data/karyotype/karyotype.human.hg38.txt
         
         选填：
         --color1           直方图填充颜色，circos颜色模版名称，默认：vdred
         --color2           直方图填充颜色，circos颜色模版名称，默认：vdblue
