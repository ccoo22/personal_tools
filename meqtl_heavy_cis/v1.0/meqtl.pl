...
my ($snp_file, $exp_file, $sample_file, $output_dir, $cis_distance, $cis_pvalue, $report_pvalue, $snp_add_col, $exp_add_col, $anno_thread, $SOFT_RSCRIPT, $SOFT_RLIB, $if_help);
GetOptions(
...
Options: 必填
        ...
        --snp_file/-s     SNP分型文件，前三列数据固定，分别是：snp_id chr pos。后面的列可以是：注释列、样本列。
                          样本基因型格式要求： “A/T” 这种类型的数据，如果缺失，空白即可
...
Options: 可选
        --cis_distance    cis分析时的距离限制，默认： 1e6
        --cis_pvalue      cis分析时，pvalue过滤阈值， 大于该阈值的结果不会输出， 默认： 0.05
        --report_pvalue   分析结果导入excel表格时，pvalue过滤阈值，大于该阈值的结果不会输出，因为结果可能会非常多，excel表格放不下。默认： 0.001 
        ...
        --anno_thread     QTL原始结果添加注释时，使用的并行线程数量。 默认： 10
...
$exp_file    = File::Spec->rel2abs($exp_file);
$sample_file = File::Spec->rel2abs($sample_file);
...
system("$SOFT_RSCRIPT $DEFAULT_SOFT_MEQTL --cis_p  $cis_pvalue --cisdist $cis_distance --thread $anno_thread --output $result_cis --exp_file  $exp_cis_prefix.raw.txt --exp_loc_file $exp_cis_prefix.pos.txt --snv_file $snp_cis_prefix.code.txt --snv_loc_file $snp_cis_prefix.pos.txt  $snp_anno_para $exp_anno_para --rlib $SOFT_RLIB");
