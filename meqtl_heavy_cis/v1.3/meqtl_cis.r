...
library(docopt)
"Usage: meqtl_cis.r --output <file> --exp_file <file> --exp_loc_file <file> --snv_file <file> --snv_loc_file <file>  [--stop_anno --cor_file <file> --cisdist <string> --cis_p <string> --rlib <dir> --thread <int> --exp_anno <file> --snp_anno <file>]
Options:
    ...
    --cor_file <file>            环境因子矫正文件，第一列是id,环境因子ID名称要保证只含有字母、数字、下划线。 后面都是样本，数值型数据，如果缺失，空着。含有表头。务必保证样本名和顺序与snp_file中的一致。 可以不输入。
    ...
    --stop_anno                  不添加特殊注释 （计算每个配对结果中样本数量、0/1/2 样本数量，均值、中位数信息，这一步非常耗时间）
...
# 0/1/2 样本数量
snp_val_count   = apply(snp_val,1,function(x){ snp_count = table(x); paste(snp_count[c('0', '1', '2')], collapse='|')  })

# 表达量均值、中位数
gene_exp_mean   = apply(gene_exp, 1, mean)
gene_exp_median = apply(gene_exp, 1, median)

# 并行线程，统计每个配对情况下的数据统计
if(!stop_anno)
{   
    # 并行线程仍然很慢，这里改为直接提取原始数据注释
    ...
    result$sampleNum = ncol(gene_exp)
    result$snp_SampleNum   = snp_val_count[result$snps]
}
...
if(!stop_anno)
{
    ...
    result$gene_mean      = gene_exp_mean[result$gene] 
    result$gene_median    = gene_exp_median[result$gene]
}
