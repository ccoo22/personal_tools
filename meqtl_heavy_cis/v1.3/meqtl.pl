...
my $DEFAULT_READ_ME_STOP   = SCRIPT_DIR. "/readme.stop_anno.txt";

# 参数输入
my $ARGV_INFO = join " ", @ARGV;
my ($snp_file, $exp_file, $cor_file, $sample_file, $output_dir, $snp_model, $snp_a1_a2, $cis_distance, $cis_pvalue, $report_pvalue, $snp_add_col, $exp_add_col, $anno_thread, $stop_anno, $boxplot_top, $plot_y_lab, $SOFT_RSCRIPT, $SOFT_RLIB, $if_help);
GetOptions(
...
Options: 可选
        --snp_model       snp量化模型，即线性分析时，如何量化基因型，支持三种： add/dom/rec 对应 加性/显性/隐性   [默认： $DEFAULT_SNP_MODEL] 
        --snp_a1_a2       三列数据，分别是snp_id A1 A2, 包含表头。表头名称随意。该文件必须包含所有在snp_file中的snp位点。指定snp的A1 A2等位基因（参考plink）， A1=minor allele, A2=major allele。
                          如果不提供该文件，流程会根据snp_file 和 sanple_file信息，统计每一个snp位点的maf，并根据maf信息确定A1 A2等位基因，用于量化。
                          如果提供了，则根据该文件指定的A1 A2进行量化。
                          一般来说，我们会指定A1 = alt allele , A2 = ref allele
        ...
        --anno_thread     QTL原始结果添加注释时，使用的并行线程数量 [默认： $DEFAULT_ANNO_THREAD] 
                          QTL原始结果比较简陋，只有简单的pvalue等信息，所以我们自己对结果添加注释（计算每个配对结果中样本数量、0/1/2 样本数量，均值、中位数信息）。但是pair结果较多，注释会很慢，引入了多线程功能。
        --stop_anno       不做QTL特殊注释，因为太耗时间。
...
$snp_model     = $DEFAULT_SNP_MODEL  if(not defined $snp_model);
...
die "[Error] snp_model 参数只能是 add/dom/rec 中的一种" if($snp_model ne 'add' and $snp_model ne 'dom' and $snp_model ne 'rec');
...
prepare_snp($snp_file, \%hashSample, $snp_cis_prefix, $snp_model, $snp_a1_a2);
...
my $cor_para = (defined $cor_file) ? "--cor_file $cor_txt" : "";
...
system("$SOFT_RSCRIPT $DEFAULT_SOFT_MEQTL --cis_p  $cis_pvalue --cisdist $cis_distance --thread $anno_thread   $stop_anno --output $result_cis --exp_file  $exp_cis_prefix.raw.txt $cor_para --exp_loc_file $exp_cis_prefix.pos.txt --snv_file $snp_cis_prefix.code.txt --snv_loc_file $snp_cis_prefix.pos.txt  $snp_anno_para $exp_anno_para --rlib $SOFT_RLIB");
...
sub prepare_snp{
    ...
    my $snp_model      = shift @_;
    my $snp_a1_a2      = shift @_;
    my @samples        = sort {$hashSample{'DATA'}{$a}{'sort_value'} <=> $hashSample{'DATA'}{$b}{'sort_value'}}keys %{$hashSample{'DATA'}};
    ...
    my $a1_title_name = "";
    my $a2_title_name = "";
    if(defined $snp_a1_a2)
    {
        %hashA1A2 = read_matrix($snp_a1_a2, 0);
        $a1_title_name = $hashA1A2{'HEAD'}->[1];
        $a2_title_name = $hashA1A2{'HEAD'}->[2];
    }
    ...
    # 确定 ref / alt
    if(not defined $snp_a1_a2)
    {
        print "define ref/alt by maf \n" ;
    }else{
        print "define ref/alt by snp_a1_a2 file \n" ;
    }
    ...
    foreach my $snp_id(keys %hashAllele)
    {
        ...
        if(defined $snp_a1_a2)
        {   
            die "[Error] snp_a1_a2文件中丢失 $snp_id 的定义\n" if(not defined $hashA1A2{'DATA'}{$snp_id});
            $ref = $hashA1A2{'DATA'}{$snp_id}{$a2_title_name};
            $alt = $hashA1A2{'DATA'}{$snp_id}{$a1_title_name};
           
            die "[Error] snp_file文件中，位点 $snp_id 包含的基因型有 @alleles, snp_a1_a2文件中却是 $ref $alt\n" if(not ($ref ~~ @alleles and $alt ~~ @alleles));
        }
        ...
        if($snp_model eq 'dom')
        {
            ...
            $hashCode{$snp_id}{"$ref/$alt"} = 1;
            $hashCode{$snp_id}{"$alt/$ref"} = 1;
            $hashCode{$snp_id}{"$alt/$alt"} = 1;
        }
        if($snp_model eq 'rec')
        {
            $hashCode{$snp_id}{"$ref/$ref"} = 0;
            $hashCode{$snp_id}{"$ref/$alt"} = 0;
            $hashCode{$snp_id}{"$alt/$ref"} = 0;
            $hashCode{$snp_id}{"$alt/$alt"} = 1;
        }
        $hashCode{$snp_id}{""} = "";  # 空字符
        $hashCode{$snp_id}{" "} = "";  # 空字符
...
sub check_sample{
    ...
    my %hashDup;
    my @dups = grep{ $hashDup{$_}++; $hashDup{$_} > 1 } split /\t/, $line1; # 防止重复表头
    close FILE;
