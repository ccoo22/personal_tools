...
{   
    ...
    {
        my $pca            = ($diff_model eq "") ? "$input/$sub_compare_name/pca.pdf"            : "$input/$sub_compare_name/pca.diff_compound.pdf";
        my $pca_biplot     = ($diff_model eq "") ? "$input/$sub_compare_name/pca_biplot.pdf"     : "$input/$sub_compare_name/pca_biplot.diff_compound.pdf";
        my $pca_coordinate = ($diff_model eq "") ? "$input/$sub_compare_name/pca_coordinate.csv" : "$input/$sub_compare_name/pca_coordinate.diff_compound.csv";
        my $check = check_result_file('txt', $pca_coordinate, 'pdf', $pca, $pca_biplot); # 检查文件是否异常
        if($check eq "")
        {
            system("cp $pca $pca_biplot $pca_coordinate $output_sub_compare_dir/");
            print "[Good] PCA $diff_model is OK [$group_list][$sub_compare_name]\n";
            $is_ok_sub++;
            $is_ok_compare++;
        }else
        {
            print "[Error] PCA $diff_model error [$group_list][$sub_compare_name] $check \n";
            $is_ok = 0;
        }
    }else
    {
        print "[Skip] in PCA $diff_model, sample count is not over 2 or compound_count is not over 2 in [$group_list][$sub_compare_name]\n";
    }
    # 当前子分组没有结果
...
{
    print "[Error] PCA $diff_model 有问题，请仔细核查 \n";
}
