...
Options: 必填
        ...
        --input/-i               代谢物heatmap分析输入路径， 例如： backup/metabolite/Group-A1_vs_A2_vs_A3/heatmap
        --output/-o              结果输出目录
...
{   
    ...
    {
        print "[Error] in heatmap diff_model, lost $diff_compound_id [$group_list][$sub_compare_name]\n";
    ...
    {   
        my $heatmap = "$input/$sub_compare_name/heatmap.diff_compound.pdf";
        my $check = check_result_file('pdf', $heatmap);

        if($check eq "")
        {
            system("cp $heatmap $output_sub_compare_dir/");
            print "[Good] heatmap diff_model is OK [$group_list][$sub_compare_name]\n";
            $is_ok_sub++;
            $is_ok_compare++;
        }else
        {
            print "[Error] heatmap diff_model error [$group_list][$sub_compare_name]\n";
            $is_ok = 0;
        }
    }else
    {
        print "[Skip] in heatmap diff_model, sample count is not over 1 or compound_count is not over 1 in [$group_list][$sub_compare_name]\n";
    }
...
{
    print "[Error] heatmap diff_model 有问题，请仔细核查 \n";
}
