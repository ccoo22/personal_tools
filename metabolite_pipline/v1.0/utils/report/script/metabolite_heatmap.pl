...
foreach my $sub_compare_name(sort keys %hashGroup)
{
    ...
    {   
        my $heatmap = ($diff_model eq "") ? "$input/$sub_compare_name/heatmap.pdf" : "$input/$sub_compare_name/heatmap.diff_compound.pdf" ;
        ...
        {
            print "[Error] heatmap $diff_model error [$group_list][$sub_compare_name] $check\n";
            $is_ok = 0;
        }
    }else
    {
