...
foreach my $sub_compare_name(sort keys %hashGroup)
{
    ...
    if($sample_count > 2 and $compound_count > 2 and scalar(@group_names) > 1)
    {
        my $plsda            = ($diff_model eq "") ? "$input/$sub_compare_name/plsda.pdf"            : "$input/$sub_compare_name/plsda.diff_compound.pdf";
        my $plsda_coordinate = ($diff_model eq "") ? "$input/$sub_compare_name/plsda_coordinate.txt" : "$input/$sub_compare_name/plsda_coordinate.diff_compound.txt";
        my $plsda_vip        = ($diff_model eq "") ? "$input/$sub_compare_name/plsda_vip.pdf"        : "$input/$sub_compare_name/plsda_vip.diff_compound.pdf";
        my $plsda_vip_txt    = ($diff_model eq "") ? "$input/$sub_compare_name/plsda_vip.txt"        : "$input/$sub_compare_name/plsda_vip.diff_compound.txt";
        ...
        {
            print "[Error] PLSDA $diff_model error [$group_list][$sub_compare_name] $check\n";
            $is_ok = 0;
        }
    }else
    {
        print "[Skip] in PLSDA $diff_model, group count is not over 1 or sample count is not over 2 or compound_count is not over 2 in [$group_list][$sub_compare_name]\n";
    }
    # 当前子分组没有结果
