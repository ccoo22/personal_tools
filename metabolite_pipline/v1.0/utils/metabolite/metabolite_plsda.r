...
library(docopt)
"Usage: MetaboAnalyst_plsda -i <file> --plsda <file> --vip <file> --plsda_coordinate <file> --plsda_vip <file> --sample_group_file <file> [--id_file <file> --color <string> --feat_num <int> --show_confidence]
Options:
	...
	--feat_num <int>                   the number of feature in vip for ploting [default: 15]
	--id_file <file>                   需要绘制的特征值ID文件[default: all]" -> doc
...
# 数据前处理
data <- read.table(inputfile, sep = '\t', header = TRUE, row.names = 1, check.names = FALSE, stringsAsFactors = FALSE, quote = "") # 读取定量数据，列为样本
sample.groups <- read.table(groupfile, sep = "\t", header = FALSE, quote = "", stringsAsFactors = FALSE, colClasses = 'character') # 读取样本以及分组数据
...
res <- data.frame(feature_id, data[feature_id, 1], comp1vip, check.names = F, stringsAsFactors = FALSE)        # 输出原始数据NO. Peak 以及对应的vip值
colnames(res) <- c("ID", "Name", "vip scores")
...
names(imp.top.vec) <- top.value[,"ID"] # 编号 NO.
feat_name   <- rev(top.value[,"Name"])  # 特征名 Peak
