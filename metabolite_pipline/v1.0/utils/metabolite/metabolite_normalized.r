...
data_raw <- read.table(input, head = T, check.names = F, stringsAsFactors =F, sep = "\t", quote = "")
...
# 补充名称
data_norm <- data.frame(data_raw[,1:2, drop=F], data_norm, check.names=F)
colnames(data_norm)[1:2] = c('ID', 'Name')
