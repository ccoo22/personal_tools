...
"Usage: MetaboAnalyst_clustering -i <file> --clusterplot <file> --sample_group_file <file> --smplDist <string> --clstDist <string> [--color <string> --id_file <file>]
Options:
    -i, --input <file>                 the input file, 第一列是特征名称， each row is a feature, each column is sample
...
data <- read.table(inputfile, sep = '\t', header = TRUE, row.names = 1, check.names = FALSE, quote = "") # 读取定量数据，列为样本
