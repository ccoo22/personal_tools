...
my $clusterProfiler_self_script   = "$tools_dir/clusterProfiler_selfdb.R";    # 自建数据库的 富集分析工具
...
if(not exists $hashPahtwayDB{$species}{'pathway_desc'})
{   # 软件自己数据库分析
    system("$Rscript $clusterProfiler_script      -s $OrgDb_name $gene_list $output_dir");
}else
{   # 缺少org，只能自己建库的数据分析
    print "[warnings] 当前物种仅支持kegg数据库分析\n";
    system("$Rscript $clusterProfiler_self_script -g $gene_list --pathway_desc $hashPahtwayDB{$species}{'pathway_desc'} --pathway_geneid_map $hashPahtwayDB{$species}{'pathway_geneid_map'} --geneid_symbol_map $hashPahtwayDB{$species}{'geneid_symbol_map'} --output_dir $output_dir");
}
...
system("mv $output_dir/GO_barplot.pdf $output_dir/go.bp.pdf  $output_dir/go.cc.pdf $output_dir/go.mf.pdf $custom_figure_dir") if(not exists $hashPahtwayDB{$species}{'pathway_desc'});  # 自己建数据目前不支持GO
...
system("perl $table2excel -i $go_raw,$readme_go     -s GO,README -o $excel_go") if(not exists $hashPahtwayDB{$species}{'pathway_desc'});  # 自己建数据目前不支持GO
system("perl $table2excel -i $kegg_raw,$readme_kegg -s KEGG,README -o $excel_kegg");
system("rm    $go_raw") if(not exists $hashPahtwayDB{$species}{'pathway_desc'});  # 自己建数据目前不支持GO
system("rm    $kegg_raw");
...
sub get_enrichment_db{
    ...
    $hashPahtwayDB{"rabbit"}{'OrgDb'}                          = "ocu";
    $hashPahtwayDB{"rabbit"}{'pathway_desc'}                   = "$self_database_dir/ocu.pathway.desc.txt";
