...
sub read_pathway_conf{
    ...
    while(my $line = <FILE>)
    {
        ...
        my $color = "";
        my $title_new = "";
        foreach my $info(split /,/, $title)
        {
            my ($ko_id, $anno) = split /\s+/, $info, 2;
            $anno = '' if(not defined $anno);
            if(exists $hashColor->{$ko_id})
            {
                $color = $hashColor->{$ko_id}{'color'} if($hashColor->{$ko_id}{'color'} ne '');  # 颜色
                $anno .= "($hashColor->{$ko_id}{'symbol'})";  # 添加客户的特殊基因id，方便查看
            }
            $title_new .= "$ko_id $anno,";
        }
        $title_new=~s/,$//;

        # 记录
        $hashConf{$row}{"mark"} = $color if($shape eq "rect" and $color ne "");
        $hashConf{$row}{"title"} = $title_new;
...
sub read_kegg_enrichment{
    ...
    die "[Error] kegg_enrichment file need  'KOID' head\n" if(!'KOID' ~~ @heads);
    while(<ENRICHMENT>)
    {
        next if($_!~/\w/);
        $_=~s/[\r\n]//g;
        ...
        {   
            ...
            my $ko_id       = $ko_ids[$count];
            my $color       = exists $hashColor{$gene_id} ? $hashColor{$gene_id} : 'up';
            $hashDiffPahtway{$pathway_id}{$ko_id}{'color'} = $color;
            $hashDiffPahtway{$pathway_id}{$ko_id}{'symbol'} .= "$gene_id ,";
        }

        # 清理尾部逗号
        foreach my $ko_id(keys %{$hashDiffPahtway{$pathway_id}})
        {
            $hashDiffPahtway{$pathway_id}{$ko_id}{'symbol'} =~s/,$//;
        }
    }
    close ENRICHMENT;
...
sub help{
    my $info = "
Program: get kegg pathway download address， 根据富集分析结果，获取下载地址,基因设成红色。该脚本主要针对自定义数据库的KO通路富集分析结果分析。
Version: 2021-01-19
Contact: 129 甘斌
...
Options:
        [必填]
         --kegg_enrichment/-k    kegg_enrichment.xls 结果文件。kegg通路富集分析结果。必须包含：ID/geneID/KOID三列信息
         --output_dir/-o         输出目录

        [选填]
         --color/-c              基因颜色文件，第一列基因；第二列up/down, (up=red, down=blue)，不区分大小写。
                                 如果第二列为空，所有基因默认为red
                                 该文件没有列出来的geneID，默认也全部设为red
                                 注意：存在一个KOID包含多个不同的geneID的情况，就会导致同一个KOID即要求red，也要求blue的情况，此时，我们仅以最先出现的设定为准。
