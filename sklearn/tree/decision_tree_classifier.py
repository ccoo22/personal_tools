...
def initialize_DecisionTreeClassifier():
    """[初始化 决策树分类模型]

    Returns:
        [sklearn.tree._classes.DecisionTreeClassifier]: [sklearn.tree._classes.DecisionTreeClassifier 对象]
    """    
    clf = tree.DecisionTreeClassifier(criterion='gini'  # 不纯度计算方法 gini / entropy  # 
...
if __name__ == '__main__':
    ...
    if args.model == 'all':
        clf = initialize_DecisionTreeClassifier()
        # 训练
        ...
        # 每个特征的重要性
        feature_importance = pd.DataFrame(zip(feature_name, clf.feature_importances_), columns=['feature_name', 'feature_importances'])
        ...
        dot_data = tree.export_graphviz(clf
            ...
            ,class_names=class_name  # 一定要与分类数字id排序后对应一致
            ,filled=True
            ,rounded=True
            )
