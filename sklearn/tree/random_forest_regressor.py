...
def initialize_RandomForestRegressor():
    """[初始化 决策树回归模型]

    Returns:
        [sklearn.ensemble._forest.RandomForestRegressor]: [sklearn.ensemble._forest.RandomForestRegressor 对象]
    """    
    rfr = ensemble.RandomForestRegressor(n_estimators=100
            ,criterion='mse'  # "mse"使用均方误差mean squared error(MSE), “friedman_mse”使用费尔德曼均方误差, "mae"使用绝对平均误差MAE（mean absolute error）
...
if __name__ == '__main__':
    ...
    if args.model == 'all':
        rfr = initialize_RandomForestRegressor()
        # 训练
        rfr = rfr.fit(data, target)
        # 导入测试集，计算R2值，越大越好，最大值为1
        score = rfr.score(data, target)
        # 预测
        predict_train= rfr.predict(data)
        ...
        # 每个特征的重要性
        feature_importance = pd.DataFrame(zip(feature_name, rfr.feature_importances_), columns=['feature_name', 'feature_importances'])
    ...
    if args.model == 'train_test':
        ...
        # 预测
        predict_train = rfr.predict(Xtrain)
        predict_test  = rfr.predict(Xtest)
    ...
    if args.model == 'train_test_cv':
        rfr = initialize_RandomForestRegressor()
        cv = 10
        scores = cross_val_score(rfr, data, target, cv=cv)
