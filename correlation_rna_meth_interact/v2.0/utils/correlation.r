...
# (2) 计算
result = matrix(nrow = nrow(data_rna) * nrow(data_meth), ncol = 11);
...
for(row_rna in 1:nrow(data_rna))
{
    ...
    {   
        row = row + 1
        data <- cbind(rna=as.numeric(data_rna[row_rna, 5:ncol(data_rna)]), meth=as.numeric(data_meth[row_meth, 5:ncol(data_meth)]))
        ...
        result[row, ] = c(as.matrix(data_rna[row_rna, 1:4])[1, ], as.matrix(data_meth[row_meth, 1:4])[1,], nmiss, pvalue, estimate )
    }
}
