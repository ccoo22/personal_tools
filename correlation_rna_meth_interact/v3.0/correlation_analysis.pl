...
my @heads = ('RNA_ID', 'RNA_chr', 'RNA_start', 'RNA_end', 'METH_ID', 'METH_chr', 'METH_start', 'METH_end', 'NMISS', 'Pvalue', 'Estimate', 'METH_RNA_dist');
...
{   
    ...
    while(<FILE>)
    {
		...
		my $METH_RNA_dist = $meth_start - $rna_start;
...
Version: 2019-11-18 v3.0 
Update : 结果里增加一列METH_RNA_dist(甲基化-RNA的起始位点);
		 增加曼哈顿图、QQ图、散点图（横坐标是距离，纵坐标是-log10(pvalue)）
Contact: 294 wangly
