...
library(docopt)
"Usage: split_data.r --rna_file <file> --meth_file <file> --sample_file <file> --interact <file> --output_dir <dir> [--thread <int>]
Options:
    ...
    --thread <int>                并行线程数量，加快速度 [default: 10]
...
# (3) 确认存在交集的基因、转录本
genes = unique(data_interact[, 4])
cl <- makeCluster(thread)
clusterExport(cl, c("data_interact", "data_rna", "data_meth", "output_dir"))  # 通知并行线程的共享变量
parLapply(cl, genes, function(gene){
        # 交集甲基化
