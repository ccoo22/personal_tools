...
sub get_rna_extend_bed{
	...
	while (<IN>){
		$_=~s/[\r\n]//g;
		my ($id, $chr, $start, $strand) = split /\t/, $_;
		my $start_up = $start - $up_extend;
		my $start_down = $start + $down_extend;
		$start_up = 0 if($start_up <0);
		next if($start_down < 0);
		print OUT "$chr\t$start_up\t$start_down\t$id\n";
	}
	close IN;
	close OUT;	
}
...
Version: 2019-11-19 v4.0 
Update : 表达量矩阵输入文件修改为：第一列为ID,第2-4列为染色体、起始、方向
         甲基化矩阵输入文件修改为：第一列为ID,第2-3列为染色体、甲基化位点
         RNA的扩展范围以起始位置（TSS）为准，上下500k
Contact: 294 wangly
...
Options:
    必填：
        --rna/-r          表达量矩阵，每一行为基因、转录本，每一列为样本，第一列为ID,第2-4列为染色体、起始、方向。示例：/home/wangly/scripts/correlation_rna_meth_interact/v4.0/gene.xls
        --meth/-m         甲基化矩阵，每一行为位点ID，每一列为样本，第一列为ID,第2-3列为染色体、甲基化位点。示例：/home/wangly/scripts/correlation_rna_meth_interact/v4.0/methyl.xls
                          两个文件对样本顺序没有要求
    ...
    选填：
        --up_extend/-up      RNA的起始位置上调范围，默认：500000
